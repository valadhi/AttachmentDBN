...
def hidden_activation_probability(v, W, c):
    ...
    for i in range(W.shape[0]):
        activations[i] = sigmoid(c[i] + sum([W[i,j]*v[j] 
...
def visible_activation_probability(h,W,b):
    ...
    for j in range(W.shape[1]):
        activations[j] = sigmoid(b[j] + sum([W[i,j]*h[i] 
            for i in range(W.shape[0])]))
    return activations
...
def probSoftmax(input, totalinput):
    global labelBits
    label = np.zeros(labelBits)
    prob = label
    prob = [input[i]/totalinput for i in xrange(labelBits)]
    r = np.random.random()
    indx = 0
...
def samplesoftmaxlabel(weights, layr):
    global labelBits
    sumx_i = 0
    x_i = []
    for i in xrange(labelBits):
        x_i.append(np.exp(sum([weights[j,-i] * layr[j] 
            for j in xrange(weights.shape[0])]))) # might be i instead of -labelBits
        sumx_i += x_i[i]
    return probSoftmax(x_i, sumx_i)

def rbmUpdate(x,W,b,c,lr=0.1):
    global labelBits
    #### split x W b and c into image and label
    x_label = x[-labelBits:]
    x = x[:-labelBits]
    W_label = W[:, -labelBits:]
    W = W[:, :-labelBits]
    ...
    nwW_l, nwb_l, nwc_l = rbmLabelUpdate(x_label, 
        W_label, b_label, c_label, lr)

    #print type(new_b)
    #print type(nwb_l)
    ## concat image and label params and return TODO
    new_b = np.concatenate((new_b, nwb_l), axis = 1)
    new_W = np.concatenate((new_W, nwW_l), axis=1)
    return new_W,new_b,new_c,error

def rbmLabelUpdate(x, W, b, c, lr=0.1):
    h1 = sample_hidden_state(x,W,c)
    v2 = samplesoftmaxlabel(W, h1)
    #q_v2 = probSoftmax(h1,W)
    q_h2 = hidden_activation_probability(v2,W,c)
    new_b = b + lr*(x-v2)
    new_c = c + lr*(h1-q_h2)
    a = np.outer(h1,x)
...
def createData():
    ...
    for i in emotionArray:
        classLabels[i] = samplesoftmaxlabel(np.zeros((2,900)), np.zeros(900)).tolist()
        #print i
        #label = samplesoftmaxlabel(np.zeros((2,900)), np.zeros(900))
        for j in range(training_datapoints):
            parsedImg = parseImage(str(i) + "\\" + str(i)+
                "-image"+str(j)+".jpg", i)
            #print type(parsedImg)
...
def createImagefromWeights(data, hidden_unit_nr, emotion, gen):
    ...
    #print type(randVisState),"randVisState",len(randVisState)
    #print type(vect_),"vect_",vect_.shape
    #print emotion
    ###### run 4 steps of CD
