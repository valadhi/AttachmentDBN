...
nrDataPoints = 30
imageSize = 20
emotionArray = ["smiley", "sad", "unsure"]
labelBits = imageSize**2
associations = {"smiley": "sad", "unsure": "smiley", "sad": "unsure"}
classLabels = {}
totHidden = 3
...
def createData():
    ...
    for i in emotionArray:
        classLabels[i] = parseImage(str(associations[i]) + "\\" + 
...
def CD_1Label(vis, Wght, c, b):
    ...
    return sample
# TODO check that this method functions properly


def reconstructImagefromWeights(data, hidden_unit_nr, emotion, gen):
    ...
    img = img.rotate(-90)
    img.save(emotion + "\\LabelledOutputCompare"+str(gen)+".jpg")
...
def start():
    ...
    if(os.path.isfile(tempWeights) and os.path.isfile(tempLabels)): 
        ...
        for i in range(len(emotionArray)):
            for j in range(13):
                reconstructImagefromWeights(data, i, emotionArray[i], j)
    else:
        if(os.path.isfile(tempWeights) or os.path.isfile(tempLabels)):
            ...
            print imageSize**2 + labelBits
            r = RBM(imageSize**2 + labelBits, totHidden)
            r.train(training_data ,max_iterations=80,lr=0.1)
            ...
            reconstructImagefromWeights(np.array([[r.W], [r.c]]), 0)
