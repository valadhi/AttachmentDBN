...
'''
  ...
  trainVectors, trainLabels = readother.read()
  testingVectors, testLabels = readother.readTest()
  ...
  #savetrain(trainingScaledVectors, nrData*2)
  # Train the network
  if args.train:
    ...
    print "weightshape",net.weights.T.shape
  ...
  randoshape = testingVectors[0,:].shape
  rando = np.random.random_sample(randoshape)
  rando1 = np.random.random_sample(randoshape)
  rando2 = np.random.random_sample(randoshape)
  ...
  recon2 = net.reconstruct(rando1, label2,3)
  recon3 = net.reconstruct(rando2, label3,3)
  recon11 = net.reconstruct(rando, label1,300)
  recon21 = net.reconstruct(rando1, label2,300)
  recon31 = net.reconstruct(rando2, label3,300)
  saveImage(recon1, "1")
  saveImage(recon2, "2")
  saveImage(recon3, "3")
...
# save training data associations
def savetrain(datatrain, t):
  print "asdfadsfwaefw",datatrain.shape
  for i in xrange(10):
    plt.imshow(vectorToImage(datatrain[i*30,:], (50, 25)), cmap=plt.cm.gray)
    plt.axis('off')
